...
int main(void)
{
    ...
    chunk_t fake = { 0 };
    fake.fd_nextsize = (chunk_p)0xdeadbeef;

    chunk_p c1 = (chunk_p)((char *)p1 - 0x10);
    c1->bk_nextsize = &fake; // c1->bk_nextsize->fd_nextsize != c1

    // put p2 into largebin
    free(p2);
    malloc(0x800); // malloc(): largebin double linked list corrupted (nextsize)
    return 0;
}